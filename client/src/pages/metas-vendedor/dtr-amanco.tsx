    ...
    if (isError || !data) {
        return (
            <Alert variant="destructive" className="mt-4">
                ...
                <AlertDescription>Não foi possível carregar os dados da Campanha DTR Amanco.</AlertDescription>
            </Alert>
        );
    }
                ...
                <div className="flex items-start gap-3 w-full">
                    ...
                    <div className="flex-1">
                        <h2 className={`text-xl font-bold ${el.participando ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'}`}>
                            {el.participando ? "Você está Elegível!" : "Ainda não elegível"}
                        </h2>
                    ...
                    <CardHeader className="pb-3">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <CardDescription className="text-xs">Faturamento Amanco no trimestre</CardDescription>
                            </div>
                ...
                <Card className="shadow-sm">
                    ...
                    <CardContent>
                        ...
                        <div className="text-sm">
                            {mx.status_ok
                                ? <span className="text-emerald-600 font-medium">✓ Acima da meta de participação.</span>
                                : <span className="text-amber-600 font-medium">Você precisa vender mais conexões Amanco.</span>}
                        </div>
                    </CardContent>
                </Card>

                {/* Crescimento vs. Ano Anterior — SEM valores monetários */}
                <Card className="shadow-sm">
                    <CardHeader className="pb-3">
                        <div className="flex justify-between items-start">
                            <CardTitle className="text-base flex items-center gap-2">
                                <TrendingUp className="w-4 h-4 text-amber-500 flex-shrink-0" /> Crescimento vs. Ano Anterior
                            </CardTitle>
                        </div>
                        <CardDescription className="text-xs">Seu crescimento em relação ao ano anterior (Informativo)</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="flex items-center gap-3 mb-2">
                            <Badge className={`text-base px-3 py-1 ${cv.crescimento_percentual >= 0 ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300" : "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300"}`}>
                                {cv.crescimento_percentual >= 0 ? "+" : ""}{cv.crescimento_percentual.toFixed(1)}%
                            </Badge>
                            <span className="text-sm text-muted-foreground">de crescimento pessoal</span>
                        </div>
                        <Progress value={Math.min(Math.max(cv.crescimento_percentual + 100, 0), 200) / 2} className="h-3 bg-amber-100 [&>div]:bg-amber-500" />
                    </CardContent>
                </Card>
                    ...
                    <CardHeader className="pb-3 border-b bg-muted/30">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <CardDescription className="text-xs">Crescimento Global da Loja vs. Ano Anterior</CardDescription>
                            </div>
