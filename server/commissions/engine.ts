...
export async function calculateCommission(
  ...
  if (companyId === "all") {
    ...
    netSalesRow = await pgGet<{ total: number }>(
      ...
      [salespersonId, startDate, endDate, parseInt(companyId)]
    );
  }
  ...
  let goalMonthly = 0;
  if (companyId === "all") {
    goalMonthly = monthlyGoals.reduce((s, g) => s + g.targetValue, 0);
  } else {
    const g = monthlyGoals.find(g => g.companyId === companyId);
    goalMonthly = g?.targetValue ?? monthlyGoals.reduce((s, g) => s + g.targetValue, 0);
  }
  ...
  for (const wg of weeklyGoals) {
    ...
    if (companyId === "all" || !wg.companyId || wg.companyId === "all") {
      ...
      weekSalesRow = await pgGet<{ total: number }>(
        `SELECT COALESCE(SUM("TOTALVENDA_LINHA"), 0) as total FROM cache_vendas
         WHERE "IDVENDEDOR" = ? AND "DT_MOVIMENTO" >= ? AND "DT_MOVIMENTO" <= ? AND "IDEMPRESA" = ?`,
        [salespersonId, weekStart, weekEnd, parseInt(wg.companyId)]
      );
    }
    const weekSales = weekSalesRow?.total ?? 0;
    if (weekSales >= wg.targetValue) weeksAchieved++;
  }
  ...
  for (const rule of rules) {
    ...
    if (rule.type === "base_monthly") {
      ...
      const bracket = thresholds.find(t => goalAchievement >= t.from && goalAchievement <= t.to);
    ...
    if (rule.type === "accelerator" && goalAchievement >= (cfg.triggerFrom ?? 110)) {
      const rate = cfg.rate ?? 0;
      const extraBase = eligibleSales * ((goalAchievement - (cfg.triggerFrom ?? 110)) / 100);
      ...
      steps.push({
        ...
        rate,
        amount,
        detail: `Acelerador a partir de ${cfg.triggerFrom ?? 110}% → excedente R$ ${fmtBRL(extraBase)} × ${rate}%`,
      });
    }
  ...
  const totalAmount = baseAmount + bonusAmount + strategicAmount - deductions;
...
function getWeekRange(year: number, month: number, weekNum: number): { weekStart: string; weekEnd: string } {
  ...
  return { weekStart: fmt(wStart), weekEnd: fmt(wEnd) };
}
